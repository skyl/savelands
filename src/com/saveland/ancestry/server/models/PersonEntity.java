...
public class PersonEntity {
	...
	private final static Logger logger = Logger.getLogger(PersonEntity.class.getName());
	private static final DatastoreService datastore = DatastoreServiceFactory.getDatastoreService();

	/*
	 * Create a new person or update and existing person
	 */
	public static Person savePerson(Person person) throws EntityNotFoundException {
	...
	public static Person createPersonFromEntity(Entity e) {
		...
		if (properties.get("fatherKey") != null) {
			p.fatherKey = KeyFactory.keyToString((Key) properties.get("fatherKey"));
		}
		if (properties.get("motherKey") != null) {
			p.motherKey = KeyFactory.keyToString((Key) properties.get("motherKey"));
		}
